  ...
  return (
    <section ref={containerRef} className="relative h-[300vh] bg-white">
      <div className="sticky top-0 h-screen w-full flex items-center px-6 lg:px-24 overflow-hidden">
        ...
        <div className="hidden lg:flex flex-col justify-center h-full border-l border-black/5 pr-12 gap-24 relative z-20">
          {steps.map((_, i) => (
            <div
              key={i}
            ...
            >
              <div className={`w-2 h-2 rounded-sm rotate-45 transition-all duration-500 ${i === activeStep ? 'bg-[#F58220] scale-150' : 'bg-black/10'}`} />
              <span className={`type-label transition-all duration-500 ${i === activeStep ? 'text-black opacity-100' : 'text-black/20'}`}>Step 0{i + 1}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 lg:pl-24 relative z-10">
          <div className="max-w-3xl mx-auto lg:mx-0 text-center lg:text-left">
            <div className="flex items-center gap-4 mb-10">
              ...
              <div className="flex-1 h-[1px] bg-black/5 hidden lg:block" />
            </div>

            <h3 className="text-[clamp(2.2rem,7vw,4.5rem)] font-bold text-[#1c1c1b] mb-8 tracking-tighter leading-[1] transition-all duration-700">
              {steps[activeStep].hook}
            </h3>

            <p className="text-black/30 text-lg md:text-xl font-light leading-relaxed max-w-xl transition-all duration-700 border-black/10 lg:border-l-2 lg:pl-10">
